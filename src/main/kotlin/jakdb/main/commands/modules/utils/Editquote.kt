...
import jakdb.data.mysql.editQuoteByName
...
class Editquote(command: String, rank: Int, test: Boolean,
    ...
    override fun execute(channel: MessageChannel, msg: Message, user: User, args: String) {
        ...
        if(isQuoteExistsByName(guild!!, name)) {
            editQuoteByName(guild, name, args.substring(name.length+1))
            channel.sendMessage(getMessage(lang!!, "Utils", "editquote", replace).build()).queue()
