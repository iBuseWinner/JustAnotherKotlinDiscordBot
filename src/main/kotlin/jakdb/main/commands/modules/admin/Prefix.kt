...
import jakdb.data.mysql.getCmdPrefix
import jakdb.data.mysql.getGuildLang
import jakdb.data.mysql.setCmdPrefix
import jakdb.jda
...
class Prefix(command: String, rank: Int, test: Boolean,
    ...
    override fun execute(channel: MessageChannel, msg: Message, user: User, args: String) {
        ...
        if(args.isNotEmpty()) {
            replace["<guild.command.prefix>"] = args
            setCmdPrefix(guild!!, args)
            channel.sendMessage(getMessage(lang!!, "Admin", "setcmdprefix",replace).build()).queue()
        } else {
            val pref = getCmdPrefix(guild!!)
            replace["<guild.command.prefix>"] = pref
            channel.sendMessage(getMessage(lang!!, "Admin", "getcmdprefix",replace).build()).queue()
        }
    }
}